...
class BaileysProvider extends ProviderClass {
    ...
    initBailey = async () => {
        ...
        try {
            ...
            sock.ev.on('connection.update', async (update) => {
                ...
                //     console.log('Debe escanear el código QR.');
                // }
     ...
     * @returns
     */
    MensajesActivos = async () => {
        // const rutaArchivos = `${this.globalVendorArgs.name}_sessions\\Envios`;
        const rutaArchivos = `Envios`;
        const rutaReporte= `Reportes\\Reporte.json`;
        // console.log(rutaArchivos)
        const watcher = chokidar.watch(rutaArchivos);

        watcher.on('add', async (path) => {

            const contenidoJSON = fs.readFileSync(path, 'utf-8');
            // console.log(path)
            // Parsea el contenido JSON a un objeto JavaScript
            const objetoJSON = JSON.parse(contenidoJSON);
            var number = objetoJSON.numero
            var message = objetoJSON.mensaje.replace(/@nombre/g, objetoJSON.nombre);
            var rutaArchivo = objetoJSON.rutaArchivo
            // console.log(message)
            var optiones = {}
            if (rutaArchivo!="") {
                optiones = {
            ...
            var estadoEnvio = false;
            var descripcionEstado = ""
            // Agregar tiempo de espera de 3 segundos
            //await new Promise(resolve => setTimeout(resolve, 4000));
            await this.sendMessage(number, message, { options: optiones }).then(() => {
                estadoEnvio = true;
                descripcionEstado = 'Mensaje enviado correctamente.'
                // console.log('Mensaje enviado correctamente.');
            })
                .catch((error) => {
                    estadoEnvio = false;
                    descripcionEstado = 'Error al enviar el mensaje:' + error
                    // console.error('Error al enviar el mensaje:', error);
                }).finally(() => {
                    // Este bloque se ejecutará siempre, ya sea que la promesa se resuelva o se rechace
                    // console.log('Finalizar aquí');
                });
            // Elimina el archivo después de extraer la información
            fs.unlinkSync(path);
            // Realiza las acciones adicionales que desees
            objetoJSON.Estado = estadoEnvio
            objetoJSON.descripcionEstado = descripcionEstado   
            // Verifica si el archivo existe
            try {
                // Verificar si el archivo existe
                if (fs.existsSync(rutaReporte)) {
                    // Si el archivo existe, leer su contenido, agregar el nuevo dato y escribir de vuelta
                    const dataReporte = fs.readFileSync(rutaReporte, 'utf-8');
                    const datosExistentes = JSON.parse(dataReporte);
                    datosExistentes.push(objetoJSON);
                    fs.writeFileSync(rutaReporte, JSON.stringify(datosExistentes), 'utf-8');
                    console.log('Datos agregados al archivo de destino.');
                } else {
                    // Si el archivo no existe, crearlo y escribir el primer dato
                    fs.writeFileSync(rutaReporte, JSON.stringify([objetoJSON]), 'utf-8');
                    console.log('Archivo de reporte creado y datos agregados.');
                }
            } catch (error) {
                console.error('Error al agregar información al archivo de reporte:', error);
            }
    ...
    mensajesExcel = async () => {
        ...
        if (datos != false) {
            const datosReporte = [
                ['Nombre', 'TELEFONO', 'MENSAJE_PERSONALIZADO', 'ESTADO', 'DESCRIPCION'],
            ];
            for (const obj of datos) {

                var number = `${obj.CODIGO_PAIS}${obj.TELEFONO}`;
                if (number.length > 0) {
                    var message = obj.MENSAJE_PERSONALIZADO.toString();
                    var estadoEnvio = false;
                    var descripcionEstado = "ENVIA APARACE NO";
                    if (obj.ENVIA == "SI") {
                        if (typeof obj.IMAGEN === 'undefined' && typeof obj.VIDEO === 'undefined' && typeof obj.ARCHIVO === 'undefined') {
                            var options = {};
                            await this.sendMessage(number, message, options).then(() => {
                                console.log('Mensaje enviado correctamente.');
                                descripcionEstado = 'Mensaje enviado correctamente.'
                                estadoEnvio = true
                            })
                                .catch((error) => {
                                    console.error('Error al enviar el mensaje:', error);
                                    descripcionEstado = 'Error al enviar el mensaje'
                                    estadoEnvio = false
                                }).finally(() => {
                                    // Este bloque se ejecutará siempre, ya sea que la promesa se resuelva o se rechace
                                    console.log('Finalizar aquí');
                            ...
                            if (typeof obj.IMAGEN !== 'undefined') {
                                if (fs.existsSync(obj.IMAGEN)) {
                                    var optiones = {
                                        media: obj.IMAGEN
                                    };
                                    await this.sendMessage(number, message, { options: optiones }).then(() => {
                                        console.log('Mensaje con Img enviado correctamente.');
                                        descripcionEstado = 'Mensaje con Img enviado correctamente.'
                                        estadoEnvio = true
                                    })
                                        .catch((error) => {
                                            console.error('Error al enviar el mensajecon img:', error);
                                            estadoEnvio = false
                                            descripcionEstado = 'Error al enviar el mensajecon img'
                                        }).finally(() => {
                                            console.log('Finalizar aquí');
                                        });
                                }
                            ...
                            if (typeof obj.VIDEO !== 'undefined') {
                                if (fs.existsSync(obj.VIDEO)) {
                                    var optiones = {
                                        media: obj.VIDEO
                                    };
                                    await this.sendMessage(number, message, { options: optiones }).then(() => {
                                        console.log('Mensaje con Video enviado correctamente.');
                                        estadoEnvio = true
                                        descripcionEstado = 'Mensaje con Video enviado correctamente.'
                                    })
                                        .catch((error) => {
                                            console.error('Error al enviar el mensajecon Video:', error);
                                            estadoEnvio = false
                                            descripcionEstado = 'Error al enviar el mensajecon Video'
                                        }).finally(() => {
                                            console.log('Finalizar aquí');
                                        });
                                }
                            ...
                            if (typeof obj.ARCHIVO !== 'undefined') {
                                if (fs.existsSync(obj.ARCHIVO)) {
                                    var optiones = {
                                        media: obj.ARCHIVO
                                    };
                                    await this.sendMessage(number, message, { options: optiones }).then(() => {
                                        ...
                                        estadoEnvio = true
                                        descripcionEstado = 'Mensaje con Archivo enviado correctamente.'
                                    })
                                        .catch((error) => {
                                            console.error('Error al enviar el mensajecon Archivo:', error);
                                            estadoEnvio = false
                                            descripcionEstado = 'Error al enviar el mensajecon Archivo'
                                        }).finally(() => {
                                            console.log('Finalizar aquí');
                                        });
                                }
                    ...
                    datosReporte.push([obj.NOMBRE, number, message, estadoEnvio, descripcionEstado])

                }
                // Utilizar una promesa para el setTimeout
                await new Promise(resolve => setTimeout(resolve, 2000));
            }
            this.crearReport(datosReporte)
        }
    ...
    crearReport = (datos) => {
        // Crear un libro de trabajo y añadir una hoja de cálculo
        ...
        XLSX.writeFile(libro, 'C:\\Users\\LENOVO\\Desktop\\EnviosWhatsapp\\REPORTE_EnviosWhatsapp.xlsx', { bookType: 'xlsx', bookSST: false, type: 'file' });
    ...
    excelData = () => {
        try {
            ...
            // const rutaArchivo = __dirname +"\\Datos.xlsx"
            const rutaArchivo = "C:\\Users\\LENOVO\\Desktop\\EnviosWhatsapp\\Datos_EnviosWhatsapp.xlsx"
    ...
    busEvents = () => [
        {
            ...
            func: ({ messages, type }) => {
                ...
                // this.emit('message', payloadPrueba);
            },
        },
        {
